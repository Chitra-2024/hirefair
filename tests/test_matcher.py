...
def test_matcher_passes_response_model_and_default_model(
    ...
    """Verify that Matcher calls generate_structured with MatchResult and default model gemini-3.5-flash-lite."""
    with patch("app.agents.matcher.generate_structured") as mock_generate:
        ...
        # Verify model is default gemini-3.5-flash-lite
...
def test_match_candidate_convenience_function(
    ...
    with patch("app.agents.matcher.generate_structured") as mock_generate:
        ...
        assert len(result.scores) == 3
        assert mock_generate.called
        assert mock_generate.call_args.kwargs["model"] == "gemini-3.5-flash-lite"
